...
for _I in {2..30}; do
 echo "Recherche en cours de tous les mots de ${_PREFIX}${_I}.rev.debut_dic qui riment dans $_LANG.UTF-8.rev.dic"
 _EXEC="exec.l${_I}.${_SUFFIX}.sh"
 >$_EXEC
 echo ">${_DIR_SUFFIX}/$_LANG.UTF-8.l${_I}.${_SUFFIX}" >> $_EXEC
 echo "cat ${_DIR_PREFIX}/${_PREFIX}${_I}.rev.debut_dic | while IFS=\";\" read -r _SEQ; do" >> $_EXEC
  echo "grep \"^\${_SEQ}\" ${_DIR_PREFIX}/$_LANG.UTF-8.rev.dic | sort -u | rev | sed -e 's/-\\\\/-/g' | tr '\\n' ';' | sed -e 's/^ //' -e 's/;\$//' >> ${_DIR_SUFFIX}/$_LANG.UTF-8.l${_I}.${_SUFFIX}" >> $_EXEC
