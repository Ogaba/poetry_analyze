...
ls "$_DATA" | while read _FIC; do
 ...
 f_gnuplot "profondeur de séquences de rimes en fin de mot" "$_DATA"
 printf "\nEOF" >> $_TMP
 chmod u+x $_TMP
 $_TMP > ${_FIC2}
 rm $_TMP 2>/dev/null
