...
ls $_DATA | while read _FIC; do
 ...
 #echo -n ", " >> $_TMP
 echo "set samples 60" >> $_TMP
 echo "set table \"outfile\"" >> $_TMP
 echo "plot \"$_FIC\" using 2:1 smooth csplines" >> $_TMP
 echo "unset table" >> $_TMP
done
# delete last comma
#sed -i 's/, $//' $_TMP
echo "" >> $_TMP
#echo "plot -550.7*x**3 + 9826.3*x**2 - 56534.1*x + 104630.9" >> $_TMP
#echo "plot 1/(0.00062*log(x)) + 0.11532" >> $_TMP
#echo "plot 11.1 *x**6 + -370.9 *x**5 + 5113.2 *x**4 + -37438.9 *x**3 + 154548.2 *x**2 + -344011.7 *x + 326064.0" >> $_TMP
echo "plot -53.5 *x**6 + 1442.6 *x**5 + -15490.3 *x**4 + 83485.4 *x**3 + -230366.8 *x**2 + 282594.9 *x + -78828.4" >> $_TMP
