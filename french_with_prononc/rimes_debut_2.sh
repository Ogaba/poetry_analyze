...
f_purge() {
echo " purge de exec.*.p*.${_SUFFIX}.sh"
rm -f exec.*.p*.${_SUFFIX}.sh 2>/dev/null
}

for _P in {1..7}; do
 for _I in {2..20}; do
   echo "Recherche en cours de rimes en debut de mot de longueur $_I avec profondeur de $_P lettres."
   _EXEC="exec.${_I}.p${_P}.${_SUFFIX}.sh"
   >$_EXEC
   echo ">${_DIR_SUFFIX}/${_PREFIX}${_I}.p${_P}.${_SUFFIX}" >> $_EXEC
   echo "cat ${_DIR_PREFIX}/${_PREFIX}${_I}.dic | while read _R1; do" >> $_EXEC
   for (( _J=1; _J<=$(($_P - 1)); _J++ )); do
    _K=$(($_J + 1))
    _L=$(($_I + $_J))
    echo "grep -E \"^\$_R${_J}([[:alnum:]]|-)\$\" ${_DIR_PREFIX}/${_PREFIX}${_L}.dic | sort -u | while read _R${_K}; do" >> $_EXEC
   done
   echo -n "echo \"" >> $_EXEC
