...
# check first getconf ARG_MAX before using
 ...
 sort -n -k2 -g $1 | sponge $1
 rm ${1}2
}

f_consolidation () {
 if [ ! -f gnuplot/rimes_fin1_debut2.lev_freq ]; then
	echo "Calcul de la somme des fréquences des fichiers lev_freq/french.UTF-8.p*.lev_freq"
	cat lev_freq/french.UTF-8.1-*.p*.lev_freq > gnuplot/rimes_fin1_debut2.lev_freq
	f_sum_column_keep_other_column gnuplot/rimes_fin1_debut2.lev_freq
 fi
 if [ ! -f gnuplot/rimes_fin4_debut3.lev_freq ]; then
 ...
 if [ ! -f gnuplot/rimes_fin5_debut6.lev_freq ]; then
 ...
 if [ ! -f gnuplot/rimes_glissantes.lev_freq ]; then
...
if [ -n "$1" ]; then
 ...
 _LIST_FILES="rimes/french.UTF-8.1-[0-9].dic.rimes"
 f_levenshtein
 echo "Traitement des fichiers de rimes french.UTF-8.1.[0-9][0-9].rimes"
 _LIST_FILES="rimes/french.UTF-8.1-[0-9][0-9].dic.rimes"
 f_levenshtein
 echo "Traitement des fichiers de rimes french.UTF-8.1.[0-9].dic.debut_rimes"
 _LIST_FILES="rimes/french.UTF-8.1-[0-9].dic.debut_rimes"
 f_levenshtein
 echo "Traitement des fichiers de rimes french.UTF-8.1.[0-9][0-9].debut_rimes"
 _LIST_FILES="rimes/french.UTF-8.1-[0-9][0-9].dic.debut_rimes"
 f_levenshtein
